            ...
            <div className='flex-none w-1/4 border border-red-500 flex flex-col gap-2'>
                <div className='w-full relative overflow-hidden'>
                    <img
                        ...
                        alt='thumbnail'
                        className={`w-full object-contain ${isPlaying ? 'rounded-full animate-rotate-center' : 'rounded-md animate-rotate-center-pause'} shadow-sm`}
                    />
                    <div className={`absolute top-0 left-0 bottom-0 right-0 hover:bg-overlay-30 text-white flex items-center justify-center ${isPlaying && 'rounded-full'}`}>
                        <span className='p-3 border border-white rounded-full'>
                            {isPlaying ? <AudioLoading /> : <BiPlay size={30} />}
                        </span>
                    </div>
                </div>
