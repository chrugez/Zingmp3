...
const SongItem = ({ thumbnail, title, artists, releaseDate, sid, order, percent }) => {
    ...
    return (
        <div
            ...
            className={`w-full flex-auto flex gap-[10px] justify-between items-center rounded-md cursor-pointer ${order ? 'text-white bg-[hsla(0,0%,100%,.07)] hover:bg-[#945ea7]' : 'text-black  hover:bg-main-200'}`}
        >
            <div className='flex gap-4'>
                {order && <span className={` text-white drop-shadow-md text-[32px] m-auto`}>{order}</span>}
